class TestJaugeNaturel {
    ...
    public void testDeplacementValeur() {
	JaugeNaturel aJauge = new JaugeNaturel(10, 1);
	java.math.BigDecimal old = aJauge.valeur();

	aJauge.incrementer();
	assert aJauge.valeur().compareTo(old.add(new java.math.BigDecimal(1))) == 0;
	old = aJauge.valeur();

	aJauge.decrementer();
	assert aJauge.valeur().compareTo(old.subtract(new java.math.BigDecimal(1))) == 0;
    }

...
class Autobus implements Bus, Transport{
	...
	for(int i = 0; i < passagers.size(); i++){
		...
		try{
		    if( passagers.get(i).nouvelArret(this, arrets) == 1)//Si le passager est sorti, on doit décaler l'indice de la boucle pour ne pas passer au dessus du passager en question
			i--;
		}
    ...
    public void allerArretSuivant(Bus bus) throws TecInvalidException{
	arrets++;
	for(int i = 0; i < passagers.size(); i++){
	    if (passagers.get(i) != null) {
		try{
		    if( passagers.get(i).nouvelArret(bus, arrets) == 1)//Si le passager est sorti, on doit décaler l'indice de la boucle pour ne pas passer au dessus du passager en question
			i--;
		}
		catch(IllegalStateException e){
		    throw new TecInvalidException("allerArretSuivant échoué", e);
